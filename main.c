...
void preOrderTravers(list *root) {
    ...
    if (root->right != NULL) {
        strcpy((char *) &code[count_stack++], "1");
...
/*void copy_function(FILE *INPUT, FILE *DESTINY){
    char buffer[1];
    ...
    for (int i = 0; i<size; i++){
        ...
        fwrite(&buffer, sizeof(char), 1, DESTINY);
    }
}*/
...
int encryption(FILE *INPUT, FILE *OUTPUT, unsigned char *CodeOfSymbol, size_t *size_source) {
    ...
    *size_source = (size_t)(*size_source + size);
    while (*size_source >= 8) {
...
/*
*записываем в массив description  "1"
*/
